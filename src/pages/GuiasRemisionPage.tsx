...
export default function GuiasRemisionPage() {
  ...
  // v2.5.68 (Fase C): el control de despacho logístico es inventario avanzado →
  ...
  useEffect(() => {
    obtenerConfig().then(cfg => {
      const mods = (cfg.licencia_modulos || "");
      setModuloLogistica(cfg.demo_activo === "1" || mods.includes("multi_almacen"));
    }).catch(() => {});
  }, []);
                ...
                {guias.map((g) => (
                  <tr key={g.id}>
                    ...
                    <td>
                      ...
                      {moduloLogistica && (g as any).despacho_estado && (() => {
                        const op = derivarEstadosNota({ estado: g.estado, despacho_estado: (g as any).despacho_estado }).operativo;
                        return (
                          <span style={{ fontSize: 9, padding: "1px 6px", borderRadius: 3, fontWeight: 600, color: op.color, border: `1px solid ${op.color}`, marginLeft: 4 }}>
                            {op.texto}
                          </span>
                        );
                      })()}
                    ...
                    <td>
                      <div className="flex gap-1">
                        ...
                        {g.estado_sri === "AUTORIZADA" && (
                          <span style={{ fontSize: 10, padding: "2px 8px", fontWeight: 600, color: "var(--color-success)" }}
                            title="Esta nota tiene una Guia de Remision autorizada por el SRI (ver en Contabilidad)">
                            ✓ Guía SRI
                          </span>
                        )}
                      </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}
          ...
          <div className="card" style={{ width: 550, maxHeight: "85vh", overflow: "auto" }}>
            ...
            <div className="card-body">
              ...
              {/* v2.5.68: estados DERIVADOS automáticos, separados por contexto
                  (operativo / comercial / tributario). No se guardan en BD. */}
              {(() => {
                const e = derivarEstadosNota({
                ...
                const chip = (label: string, s: { texto: string; color: string }) => (
                  <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                    ...
                    <span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 4, fontWeight: 600, color: s.color, border: `1px solid ${s.color}`, alignSelf: "flex-start" }}>
                      {s.texto}
                    </span>
                  </div>
                );
                return (
                  <div style={{ display: "flex", gap: 16, marginBottom: 10, flexWrap: "wrap" }}>
                    {chip("Operativo", e.operativo)}
                    {chip("Comercial", e.comercial)}
                    {chip("Tributario", e.tributario)}
                  </div>
                );
              })()}
