...
pub fn cerrar_caja(
    ...
    // CALCULO DEL ESPERADO (v2.3.44+): SIEMPRE usar el recalculado.
    // Antes usabamos `monto_esperado_stored.max(recalculado)` como anti-fraude, pero
    // eso causaba descuadres falsos cuando el stored quedaba desincronizado (ej:
    // bug viejo donde gastos NO restaban del stored). El recalculado es la unica
